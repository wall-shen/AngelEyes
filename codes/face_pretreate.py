...
def pick_single(fileName, targetName, *suffix):
    ...
    if os.path.exists(fileName):
        ...
        for image in images:
            # 后缀名判断
            if endwith(image, suffix):
                img = pretreat_img_internet(fileName + os.sep + image)
                # 创建目录
...
def test_pick_method(fileName):
    ...
    count3 = 0  # haar
    for file in files:
        images = os.listdir(fileName + os.sep + file)
        for image in images:
            count_sum += 1
            cnn = 0
            dlib = 0
            open = 0
            img = cv.imread(fileName + os.sep + file + os.sep + image)
            # cnn模块识别
    ...
    print('cnn:', float(count1)/float(count_sum))
    print('hog:', float(count2)/float(count_sum))
    print('haar:', float(count3)/float(count_sum))
...
if __name__ == '__main__':
    ...
    test_pick_method('/media/wall/F6F4D817F4D7D7C7/image/pretreat_test')
